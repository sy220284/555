...
受控合并器仅接受与候选 head SHA、workflow run ID、check suite ID 及精确 job 集合一致的最新 `repository-gates` 成功记录，并发布 `source-gate-run` 来源状态。主线复验使用同一个 run ID 回查，禁止按检查名称复用历史成功结果。
