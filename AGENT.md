...
AI默认报告事实和执行授权任务，不得依据比赛经过时间推动玩家总攻、撤退或升级。战略建议必须可关闭且不能自动执行。
...
合法秒数只用于动作/技术过程：建造、研究、冷却、占领作业、整备、重连、投票、过场等。

模式节奏必须由资源、工业、战略控制、兵力、交通、情报、后勤、算力和玩家投资自然产生。

自定义限时娱乐规则必须使用独立 `RULESET_`，不得进入标准排位。
...
前线/战区地图必须支持动态 `ControlRegion` 和多战略轴线；禁止为了实现方便退化成固定节点链。

## 15. 战役

24关以 `CAMPAIGN_MISSION_SPECS.md` 为准；缺省初始部队、AI姿态、增援、对白/过场遵循 `CAMPAIGN_NARRATIVE_CAST.md` 的默认契约。

关键脚本必须状态条件+超时兜底；跳过过场不能改变任务结果。战役剧情可以存在时间事件，但多人标准规则不得复制战役倒计时节奏器。
...
最终原则：**好玩优先于写实，清晰优先于复杂；玩家决定战争节奏，系统提供战争规律；技术服从冻结栈，性能靠架构/算法解决，任何状态必须如实汇报。**