...
> 本文件是仓库唯一的人工维护完整技术文档。仓库原有分散的架构说明、子系统说明、包说明、用户指南、开发手册、双语文档与历史 Agent Note 已全部读取、去重并收敛到这里。
...
- [0. 当前基线与文档收敛范围](#0-当前基线与文档收敛范围)
- [1. 项目定位](#1-项目定位)
- [2. 总体架构](#2-总体架构)
- [3. 启动、安装与运行](#3-启动安装与运行)
- [4. 智能体、会话、轮次与步骤](#4-agent会话轮次与步骤)
- [5. 系统提示词、上下文与模型](#5-系统提示词上下文与模型)
- [6. 工具系统与执行流水线](#6-工具系统与执行流水线)
- [7. 文件系统、命令环境、子进程、终端与后台任务](#7-文件系统shell子进程终端与后台任务)
- [8. 沙箱、权限与安全边界](#8-沙箱权限与安全边界)
- [9. 持久化、存储、附件与大输出](#9-持久化存储附件与大输出)
- [10. 工作区、设置与用户状态](#10-工作区设置与用户状态)
- [11. 网页客户端架构](#11-web-客户端架构)
- [12. 子智能体、团队、工作流与 Ralph](#12-子智能体团队工作流与-ralph)
- [13. 目标、计划、待办、问题与计划任务](#13-goalplantodo问题与计划任务)
- [14. 技能系统](#14-技能系统)
- [15. 网页搜索、抓取与外部接口](#15-web-搜索抓取mcpacpsdk-与远程接口)
- [16. 插件开发方法](#16-插件开发方法)
- [17. 防御性实现规则](#17-防御性实现规则)
- [18. 测试、构建与验收](#18-测试构建与验收)
- [19. 自动化与永久离线恢复](#19-自动化与永久离线恢复)
- [20. 工具目录](#20-工具目录)
- [21. 可加载插件配置总览](#21-可加载插件配置总览)
- [22. 会话持久事件目录](#22-会话持久事件目录)
- [23. 234 个工作区包完整索引](#23-234-个工作区包完整索引)
- [24. 核心子系统索引](#24-核心子系统索引)
- [25. 历史决策收敛索引](#25-历史决策收敛索引)
- [26. 文档清理后的仓库约定](#26-文档清理后的仓库约定)
- [27. 快速排错](#27-快速排错)
- [28. 最终维护原则](#28-最终维护原则)
...
- 合并后清理的分散人工文档：**2310 份**
- 整理完成时保留的 Markdown 功能/测试/法律资产：**185 份**

当前另将项目入口与完整技术正文拆为 `README.md`、`TECHNICAL.md` 两份，因此仓库自有 Markdown 总量较整理完成时增加 1 份；技术事实仍只在本文件维护。
...
3. **历史决策不丢。** 原 Agent Note 的题名与状态全部收录到本文“历史决策收敛索引”，仍有当前参考价值的记录同时保留摘要；需要逐行考古时使用 Git 历史。
4. **可执行 Markdown 不删。** `SKILL.md`、测试快照/夹具中的 `.md` 是程序输入或测试预期，不属于说明文档。
5. **法律文件独立。** `THIRD_PARTY_NOTICES.md` 与 `LICENSE` 保持原样。

### 0.2 术语速查

本文保留源码、协议与仓库约定中的固定名称。第一次阅读时可按下表理解，代码标识仍以源码拼写为准。

| 固定名称 | 中文说明 |
| --- | --- |
| `Agent Note` | 历史设计决策、提案、缺陷复盘或流程记录 |
| `Cordis` | 管理插件组合、服务依赖、事件和生命周期的运行时骨架 |
| `seam` | 一项可替换能力的完整边界，由服务定义、实现提供方和消费方组成 |
| `Profile` | 一套可交付、可启动的插件组合配置 |
| `surface` | 面向用户或外部调用方的展示与交互层 |
| `fiber` | Cordis 中限定插件实例、依赖和资源释放范围的生命周期作用域 |
| `spill` | 工具输出超过内联上限后写入文件的机制 |
| `Typert` | 从类型声明生成远程调用元数据、注册信息和调用入口的体系 |
| `MCP` | 模型上下文协议，用于接入外部工具和资源 |
| `ACP` | 智能体客户端协议，用于从编辑器等外部客户端驱动智能体 |
| `SDK` | 供其他程序调用工作台能力的开发工具包 |
| `LSP` | 语言服务器协议，用于跳转、引用、诊断等代码语义能力 |
| `PTY` | 可持续交互的伪终端，是持久命令行会话的底层载体 |
| `Ralph` | 由工作流和子智能体驱动的多轮迭代执行机制 |
...
自检脚本验证正式预设、5 个工作台技能、语言服务启动器、MCP 启动器、Web 组合、依赖、`mode: both`、默认 `workbench`、持久 SQLite 配置和旧旁挂目录不存在。文档收敛后，它要求根 `README.md` 作为项目入口、`TECHNICAL.md` 作为完整技术文档，不再要求 `docs/workbench.zh.md`。
...
以下是仓库当前生成目录中登记的模型可见工具全集，不代表 `workbench` 默认预设会同时启用全部工具。参数细节以对应工具包源码中的参数结构定义为最终事实源。

当前默认预设启用：按操作系统选择的命令工具、文件读写与搜索、后台任务、持久终端、技能、代码语义导航、目标与计划、内置子智能体、工作流、Ralph、多选提问、待办以及代码模式。网页搜索工具、Codex 与 Claude Code 外部子智能体在默认预设中明确关闭；实验性智能体团队也不属于默认能力。它们需要安装或配置对应实现，并在预设中显式启用。

因此，判断某项能力时按以下顺序核对：

1. 本节确认仓库是否提供对应工具包。
2. `apps/cli/config/agent-presets/workbench/agent.cordis.yml` 确认默认预设是否启用。
3. 宿主组合配置确认依赖服务和实现提供方是否已经挂载。
...
原 `.agents/notes/` 共包含大量中英成对的设计决策、提案、缺陷复盘和流程记录。中文侧的 **739 条实际决策记录**已读取；下面保留每条题名与生命周期，仍有当前参考价值的记录同时保留核心内容摘要。正文中的当前事实已经优先吸收到前 24 节，这里只承担历史检索功能；需要完整原文时使用 Git 历史。
...
今后新增公共能力时，优先更新源码类型/测试，并在 `TECHNICAL.md` 相应章节维护说明；`README.md` 只同步项目定位、快速启动和文档入口，不要重新恢复大规模分层 Markdown 文档体系。
...
> **工作台就是正式源码；完整技术说明只有这一份。**

新增能力直接进入正式源码目录、正式依赖、正式预设或正式组合；完整技术事实只在 `TECHNICAL.md` 维护，`README.md` 负责提供稳定入口。运行时技能与测试 Markdown 继续作为程序资产存在，但不再复制一套说明文档。
