...
class PlayerCommandStreamStaticTests(unittest.TestCase):
    ...
    def test_command_replay_binds_content_and_round_trips_authoritative_result(self):
        ...
        self.assertIn("equivalent command replay creation is not byte-stable", runner)
        self.assertIn("FormatVersion = 3", replay)
        self.assertIn("PlayerReplayInitialSnapshot", replay)
        self.assertIn("PlayerReplayEventRecord[] Events", replay)
        self.assertIn("GameVersion", replay)
        self.assertIn("ContentHash", replay)
        self.assertIn("TimestampUtc", replay)
        self.assertIn("DateTimeOffset timestampUtc", replay)
        self.assertIn("TimestampUtc = timestampUtc.ToUniversalTime()", replay)
