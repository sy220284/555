...
当前重放实现采用“同一权威初始配置 + 固定间隔状态哈希检查点”的方式验证整局时间线：
...
- 重放最终赢家、结束 tick 与最终哈希再次核对；
- 本次重放最终哈希仍为 `1979A9B7B4183764`；
- 1000单位压力模拟基线继续保持 `A72CA0F85058C84B`；
- 重放器不包含坦克造价、伤害、采集速率等玩法常量。

这已经建立了“状态检查点录像 + 确定性重放验证”能力，但当前参考对局仍由内置确定性策略驱动，还没有记录玩家命令流、随机事件流、内容版本迁移与长期录像文件格式，因此不得称为最终录像系统。
...
| G2 确定性模拟 | PASS（规则内核） | 10000 tick × 10，哈希一致；接桥与重放后基线未变化 | Unity程序集真实运行后需复核同级结果 |
...
| G11 完整歼灭 | PARTIAL++ | 地图驱动闭环、Unity固定步桥、实体镜像及16检查点确定性重放已建立 | Unity真实编译/运行、玩家输入/UI、玩家命令流录像、AI完整对局与正式ECS批处理未完成 |

## 当前生命周期

项目整体仍为 `implementation_bootstrap`，不得升级为 `playable`。

目前已经从“纯规则闭环”继续推进到“同源地图数据、Unity ECS 权威桥代码、确定性状态检查点重放均已落地”的阶段；但没有真实 Unity 编辑器编译和运行证据，也没有玩家命令流录像，因此不能把当前实现冒充成最终可玩版或完整录像系统。
...
7. 将当前状态检查点重放扩展为玩家命令流/随机事件流/内容版本绑定的正式录像格式，并加入AI自动完整对局；
