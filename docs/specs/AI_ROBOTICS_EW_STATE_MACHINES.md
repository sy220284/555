...
这些频率属于模拟规则，不按客户端性能变化，也不用于推动比赛进入固定阶段。
...
ZonePriority不得包含 `MatchElapsedTime`、固定阶段或“比赛太久所以进攻”变量。
...
战略候选：扩张、科技、正面进攻、侧翼、远程消耗、空袭、登陆、反后勤、反雷达、佯攻、撤退、巩固、围困、封锁、建立敌后据点。

候选效用：`ExpectedGain - ExpectedLoss - OpportunityCost + DoctrineBias`。

AI可以因为真实资源、目标暴露、兵力比、交通、后勤或玩家行为改变战略，不能因为经过X分钟强制进攻。
...
以下属于 `StrategicAdvice`，必须由玩家设置为开启后才能主动弹出：
...
即使 `StrategicAdvice` 开启，仍只是建议，不能直接执行。

AI不得使用 `elapsed_match_time` 推导“现在该进攻/升级/结束”。
...
难度倍率作用于规划机会和质量预算，不得突破各层硬最大频率形成机器反应作弊，也不能通过提前知道“比赛阶段”作弊。
...
- 同一权威输入重放关键决策一致；
- 各AI层实际更新频率符合范围；
- 长时间对局不会因为 elapsed time 自动切换战略/科技；
- `StrategicAdvice`关闭时AI不主动推动总攻/总撤退。