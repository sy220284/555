...
`MODE_QUICKWAR` 不执行科技推进过程。

进入LIVE前，本阵营所有T1—T5通用科技和正式国家/阵营特色科技全部置为 `COMPLETED`。常规模式用于限制发展路线的高阶专精槽只约束发展型模式，不限制快速战争的初始科技拥有状态。

快速战争没有研究队列，也不返还“跳过研究”的资源；完整科技属于规则集授予的战备条件。科技建筑仍然是实时系统节点，被摧毁后会让依赖该建筑运行的能力降级，但不会把已经完成的科技从数据库中删除。

详细见 `QUICKWAR_FULL_READINESS.md`。
...
快速战争跳过该发展成本并以全科技完成状态进入比赛，但互相逻辑冲突的工作模式/姿态仍不能同时激活。
...
发展型模式中，研究开始后可取消，返还70%未消耗资源；超过50%进度后取消只返还40%。主专精槽一旦锁定不可在标准排位中切换，战役可由脚本允许。

快速战争没有研究队列，因此不存在取消研究和切换主专精槽的问题。
...
快速战争还必须在模式加载时生成科技完成状态哈希，保证录像、重连和服务器恢复得到完全一致的全科技状态。