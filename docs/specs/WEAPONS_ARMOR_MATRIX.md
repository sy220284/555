...
`WPN_JP_RAILGUN_NAVAL` 为电磁炮舰专用直射武器：高能耗、需要稳定锁定；对大型水面目标和部分导弹有效，不获得爆炸范围伤害。
...
- 超级武器对友军造成50%权威伤害；
- 自伤遵循同样范围规则；
- 自定义房可覆盖，但必须进入 `ruleset_id` 和录像内容哈希。

详细模式规则见 `GAME_MODE_RULES.md`。
...
5. 所有 `WPN_` 必须在本文件或未来分拆后的正式武器注册表中存在，禁止运行时匿名武器。
