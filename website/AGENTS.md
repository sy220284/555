...
文档站只有一个规范内容源：仓库根 `TECHNICAL.md`。`website/` 只负责把它投影为静态站点与原始 Markdown 版本，不拥有第二份技术正文。

- 禁止在 `website/` 下新增复制版技术 Markdown；`website/.generated/`、`.cache/`、`.dist/` 都是可丢弃产物。
- 修改技术内容直接改根 `TECHNICAL.md`；根 `README.md` 只维护项目入口。修改站点路由、主题或投影逻辑才改 `website/` 与 `scripts/project-doc-site.ts`。
- 仓库不再维护英文镜像、`docs/` 源树或多层文档侧边栏。
- 文档站变更至少运行 `pnpm run docs:build`，并确认根 `TECHNICAL.md` 仍是唯一规范技术内容源。
